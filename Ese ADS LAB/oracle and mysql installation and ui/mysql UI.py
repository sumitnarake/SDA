...
# Connecting to DB
conn = connector.connect(host='localhost', user='2020BTECS00023', password='2020BTECS00023', database='sakila') 
c = conn.cursor()
...
window.configure(background = "gray"); # Background color of window
window.option_add("*Font", "Times 16") # Setting the font-family & font-size 

usr_name = Label(window ,text = f"Connected to DB as: rushi", background="gray").grid(row = 0, column = 1, pady=20)
...
tb_select = Label(window ,text = "Select the table: ", background="gray").grid(row = 1, column = 0, columnspan=1, padx=10, pady=10)
...
def view_tb():
        ...
        Label(newWindow ,text = f"Viewing Table - {selected_tb}", background="gray").grid(row = 0, column = 0, padx=10, pady=10)
...
def insert_tb():
    ...
    Label(newWindow ,text = f"Insert values in table: {selected_tb}", background="gray").grid(row = 0, column = 0, padx=10, pady=10)
    c.execute(f'''show columns
                  FROM {selected_tb}
                ''')
    ...
    for ind, nm in enumerate(columns):
        Label(newWindow ,text = nm, background="gray").grid(row = ind+1, column = 0, padx=10, pady=10)
        ent = Entry(newWindow)
    ...
    Button(newWindow, text="Insert Values", command=insert_val, background="gray", foreground="white").grid(row = ind+2, column = 1, pady=20, sticky='ew')
    ...
    try:
        ...
        if id is not None:
            ...
            if len(c.fetchall()) == 0:
                ...
                Label(newWindow ,text = f"Update values in table: {selected_tb}", background="gray").grid(row = 0, column = 0, padx=10, pady=10)
                c.execute(f'''show columns
                              FROM {selected_tb}
                        ''')
...
if selected_tb is not None:
    Label(window ,text = "Operations on selected table:", background="gray", font='Helvetica 18 bold').grid(row = 3, column = 0, padx=10, pady=60)

    view_btn = Button(window, text="View", command=view_tb, background="#9629ff", foreground="white", border=3).grid(row = 4, column = 0)
    insert_btn = Button(window, text="Insert", command=insert_tb, background="gray", foreground="white", border=3).grid(row = 4, column = 1, sticky='w', columnspan=1)
